...
def plot(matrix: np.ndarray) -> None:
    ...
    :return: None
    """
    sns.heatmap(matrix, annot=False, cmap="coolwarm", square=True, alpha=0.85, cbar=True, linecolor='white', linewidths=0.5,)
...
def Q(data: pd.DataFrame, target: pd.Series, k: int, lambda_ : float, bins: int = 20, with_penaly: bool = False) -> np.ndarray:
    N = len(data.columns)
    qubo = np.zeros((N, N))
...
"""if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Compute the QUBO matrix for feature selection.")
    parser.add_argument("lambda_", type=float, help="The lambda penalty for the QUBO matrix.")
    args = parser.parse_args()
    qubo_matrix = Q(new_data, target, 8, args.lambda_)
    path_to_save = "/Users/francescoaldoventurelli/qml/FeatureSelectionQubo/qubo_matrices"
    np.savetxt(path_to_save + f"/qubo_matrix_12febbraio_{str(args.lambda_)}.txt", qubo_matrix, delimiter=",")"""